...
import { d as mapAgent } from "./catalog-fns-CmNnoDaH.mjs";
import { t as authMiddleware } from "./middleware-BOQMsh2Y.mjs";
import { a as requireApproved } from "./profile-BZGV6Lgo.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/orchestrate-CeAcwdfP.js
var CLASS_SKILLS = {
	research: [
		"research",
		"citations",
...
var orchestrateTask_createServerFn_handler = createServerRpc({
	id: "e1a932d2fa8a3c8300deb2c83fc5393c638c25c7337aff134d2ef4f60b94df9c",
	name: "orchestrateTask",
	filename: "src/lib/server/orchestrate.ts"
}, (opts) => orchestrateTask.__executeServer(opts));
var orchestrateTask = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(orchestrateTask_createServerFn_handler, async ({ context, data }) => {
	return runArena(context.userId, data);
});
var getTaskArena_createServerFn_handler = createServerRpc({
	id: "928273679f33d8f12cdec18b9de2d5a69544865724ac16652672a221ea353ab2",
	name: "getTaskArena",
	filename: "src/lib/server/orchestrate.ts"
}, (opts) => getTaskArena.__executeServer(opts));
var getTaskArena = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator((taskId) => taskId).handler(getTaskArena_createServerFn_handler, async ({ context, data: taskId }) => {
	const sql = await getSql();
	const task = (await sql`
...
//#endregion
export { getTaskArena_createServerFn_handler, orchestrateTask_createServerFn_handler };
