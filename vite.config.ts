...
/**
 * Nitro relocates PGLite's wasm/data into hashed client assets, so
 * `new URL("./pglite.data", import.meta.url)` 404s under `vite preview`.
 * Copy the canonical files next to the bundled pglite module before preview
 * serves. Deployed Vercel uses Neon (`DATABASE_URL`) and never loads these.
 */
